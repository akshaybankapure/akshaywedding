...
import { get, set, mutate } from "@/lib/server/db";
...
export async function changePassword(current, next) {
  ...
  await mutate((all) => {
    all[CRED_KEY] = { ...cred, salt, hash, secret, isDefault: false };
  });
...
export async function changeUsername(password, nextUser) {
  ...
  await mutate((all) => { all[CRED_KEY] = { ...cred, user: u }; });
  await startSession();
  return { ok: true };
}
