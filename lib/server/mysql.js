...
/* MySQL grants are per-host, and "localhost" is ambiguous: Node may
   resolve it to the IPv6 loopback ::1, while the grant almost always
   covers 'user'@'localhost' / 127.0.0.1. The connection then arrives as
   user@'::1' and is refused with ACCESS DENIED even though the password
   is perfectly correct.

   So we pin to IPv4: "localhost" becomes 127.0.0.1 and the socket is
   opened with family 4. Set DB_IP_FAMILY=0 to restore automatic
   selection, or 6 to force IPv6, if you ever need it. */
...
export function getPool() {
  ...
  if (!pool) {
    pool = mysql.createPool({
      host: resolvedHost(),
      family: Number(process.env.DB_IP_FAMILY ?? 4),
