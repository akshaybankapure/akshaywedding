...
export function isConfigured() {
  if (envClean("DATABASE_URL")) return true;
  return Boolean(envClean("DB_HOST") && envClean("DB_NAME") && envClean("DB_USER"));
}
...
export function candidates() {
  ...
  if (envClean("DB_SOCKET")) {
    list.push({ label: `socket ${envClean("DB_SOCKET")}`, socketPath: envClean("DB_SOCKET") });
  }
  if (isLocal(host)) {
...
/* Values pasted into a control-panel field very often carry an invisible
   trailing space or newline. MySQL treats "pass " and "pass" as different
   passwords, so that alone produces ACCESS DENIED with everything else
   correct. We trim, and diagnostics reports when trimming changed
   something so you know it happened. */
export function envRaw(k) { return process.env[k] ?? ""; }
export function envClean(k) { return String(process.env[k] ?? "").trim(); }
export function envHadWhitespace(k) {
  const v = process.env[k];
  return typeof v === "string" && v !== v.trim();
}

/* Hostinger also documents a single DATABASE_URL. Support it, since it
   sidesteps four separate fields each of which can pick up a space. */
function fromUrl() {
  const raw = envClean("DATABASE_URL");
  if (!raw) return null;
  try {
    const u = new URL(raw);
    if (!/^mysql:?$/i.test(u.protocol.replace(":", "") + ":")) { /* fall through */ }
    return {
...
export function credentials() {
  ...
  if (url) return url;
  return {
    host: envClean("DB_HOST"),
    port: Number(envClean("DB_PORT") || 3306),
    user: envClean("DB_USER"),
    password: envClean("DB_PASSWORD"),
    database: envClean("DB_NAME"),
  };
}
...
async function negotiate() {
  ...
  if (now - lastRouteLog > 60000) {
    ...
    const c = credentials();
    console.error("[db] host=" + (c.host || "(unset)") +
      "  db=" + (c.database || "(unset)") +
      "  user=" + (c.user || "(unset)") +
      "  password=" + (c.password ? c.password.length + " chars" : "(unset)"));
    for (const k of ["DB_HOST", "DB_NAME", "DB_USER", "DB_PASSWORD"]) {
      if (envHadWhitespace(k)) {
        console.error(`[db] NOTE: ${k} had surrounding whitespace in the environment — trimmed before use.`);
      }
    }
  }
  throw err;
