...
export async function getCeremony() {
  if (isConfigured()) {
    ...
    return { akshata: Number(c?.akshata || 0), guests: Number(g?.n || 0) };
  }
  const c = (await file.get("ceremony-v1", { akshata: 0, guests: [] })) || {};
  return { akshata: c.akshata || 0, guests: (c.guests || []).length };
}

export async function addAkshata(n, deviceId) {
  const add = Math.max(1, Math.min(30, Number(n) || 1));
  if (isConfigured()) {
    // single atomic statement — hundreds of phones can tap at once
    await query(`UPDATE ceremony SET akshata = akshata + ? WHERE id = 1`, [add]);
    if (deviceId) {
      await query(`INSERT IGNORE INTO ceremony_guests (device_id) VALUES (?)`,
        [clip(deviceId, 48)]);
    }
    return getCeremony();
  }
  await file.mutate((all) => {
    const cur = all["ceremony-v1"] || { akshata: 0, guests: [] };
